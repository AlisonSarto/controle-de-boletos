<?php
  ...
  $sql = "SELECT SUM(total_a_receber) as total FROM boletos WHERE recebedor_id = $empresa_id AND recebedor_conta_id = $conta_id AND data_vencimento < '$data' AND data_vencimento LIKE '$mes%' AND situacao = 0";
  $res = $conn->query($sql);
  ...
  $sql = "SELECT SUM(total_a_receber) as total FROM boletos WHERE recebedor_id = $empresa_id AND recebedor_conta_id = $conta_id AND data_vencimento = '$data' AND situacao = 0";
  $res = $conn->query($sql);
  ...
  $sql = "SELECT SUM(total_a_receber) as total FROM boletos WHERE recebedor_id = $empresa_id AND recebedor_conta_id = $conta_id AND situacao = 1 AND data_vencimento LIKE '$mes%'";
  $res = $conn->query($sql);
  ...
  $sql = "SELECT * FROM boletos WHERE recebedor_id = $empresa_id AND recebedor_conta_id = $conta_id AND data_vencimento LIKE '$mes%' ORDER BY data_vencimento ASC";
  $res = $conn->query($sql);
  ...
  while ($row = $res->fetch_assoc()) {
    ...
    $nome_pagador = $empresas[$row['pagador_id']];
    $conta_pagador = $contas[$row['pagador_conta_id']];

    $pagador = "$nome_pagador - $conta_pagador";
